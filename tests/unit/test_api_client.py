...
def test_client_reads_and_writes_comments() -> None:
    requests: list[httpx.Request] = []

    def handler(request: httpx.Request) -> httpx.Response:
        requests.append(request)
        if request.url.path == "/tasks/task-1/comments" and request.method == "GET":
            return httpx.Response(200, headers={"X-Request-ID": "req-comments"}, json=[COMMENT])
        return httpx.Response(
            200,
            headers={"X-Request-ID": "req-comment-create"},
            json=COMMENT,
        )
    ...
    with ActaskApiClient(
        ...
        listed = client.list_comments("task-1")
    ...
    assert [comment.to_data() for comment in listed.comments] == [COMMENT]
    assert listed.request_id == "req-comments"
    assert created.comment.to_data() == COMMENT
    assert created.request_id == "req-comment-create"
    assert [(request.method, request.url.path) for request in requests] == [
        ("GET", "/tasks/task-1/comments"),
